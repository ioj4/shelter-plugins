...
const getAccount = () =>
	Object.values(
		shelter.flux.stores.SpotifyStore.__getLocalVars().accounts
	)[0];
...
export const play = (type, id) => {
	spotifyRequest(
		...
		`/play?device_id=${
			Object.values(
				shelter.flux.stores.SpotifyStore.__getLocalVars().playerDevices
			)[0][0].id
		}`,
		JSON.stringify({ uris: [`spotify:${type}:${id}`] })
	).then(
...
export const queue = (type, id) => {
	spotifyRequest(
		"POST",
		`/queue?uri=spotify:${type}:${id}&device_id=${
			Object.values(
				shelter.flux.stores.SpotifyStore.__getLocalVars().playerDevices
			)[0][0].id
		}`
